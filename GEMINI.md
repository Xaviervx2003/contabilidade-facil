...
O usuário pode trocar a paleta de destaque do sistema pela tela de **Perfil → Aparência** ou pelo botão 🎨 no header. As paletas disponíveis são:
...
Para que um elemento **responda dinamicamente** à paleta ativa, use CSS variables no lugar de valores fixos:
```css
/* Usa a cor de destaque dinâmica (muda com a paleta do usuário) */
